...
class ApiAuthentificationController extends Controller
{
   ...
   // déconnexion : invalider le JWT et révoquer les refresh tokens
   public function logout(Request $request){
       // invalider le JWT actuel (le mettre dans le blacklist si activé)
       auth('api')->logout();

       // option : si client envoie refresh_token pour le révoquer
       if ($request->has('refresh_token')) {
           $parts = explode('|', $request->input('refresh_token'));
           if (count($parts) === 2) {
               $id = $parts[0];
               $r = RefreshToken::find($id);
               if ($r) {
                   $r->update(['revoked' => true]);
               }
           }
       } else {
           // sinon, révoquer tous les refresh tokens de l'utilisateur (optionnel)
           $user = auth('api')->user();
           if ($user) {
   ...
   // info utilisateur (pour tester le token)
   public function me(){
